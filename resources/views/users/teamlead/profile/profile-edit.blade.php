        ...
        <form method="POST" action="{{ route('profiles.update', $profile->id) }}">
            ...
            <select name="device_group_id" required>
                ...
                @foreach($deviceGroups as $group)
                    <option value="{{ $group->id }}" @if($profile->device_group_id == $group->id) selected @endif>{{ $group->name }}</option>
                @endforeach
            </select>
            <select name="command_list_id" required>
                <option value="">-- Chọn Command List --</option>
                @foreach($commandLists as $cmd)
                    <option value="{{ $cmd->id }}" @if($profile->command_list_id == $cmd->id) selected @endif>{{ $cmd->name }}</option>
                @endforeach
            </select>
            <textarea name="description">{{ $profile->description }}</textarea>
            <button type="submit">Cập nhật</button>
        </form>
        <a href="{{ route('profiles.index') }}" class="back
